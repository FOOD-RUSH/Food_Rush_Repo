...
export const SimpleNotificationProvider: React.FC<NotificationProviderProps> = ({ children }) => {
  ...
  const contextValue: NotificationContextType = useMemo(() => ({
    sendOrderStatus: async (orderId: string, status: 'confirmed' | 'preparing' | 'ready' | 'delivered' | 'cancelled', restaurantName?: string) => {
      await notifications.sendOrderNotification(orderId, status, { restaurantName });
    },
    scheduleCartReminder: async (itemCount: number, restaurantName?: string, minutesFromNow: number = 25) => {
    ...
    sendNewOrder: async (orderId: string, customerName?: string) => {
      await notifications.sendOrderNotification(orderId, 'pending', { customerName });
    },
    sendPromotion: async (title: string, message: string, promotionId?: string) => {
      await notifications.sendPromotionNotification(title, message, { promotionId });
    },
    sendGeneral: async (title: string, message: string, data?: Record<string, any>) => {
