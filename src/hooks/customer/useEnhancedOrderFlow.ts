...
export const useEnhancedOrderFlow = () => {
    ...
    onSuccess: (data) => {
      ...
      if (data.status === 'confirmed' && currentStep === 'waiting_restaurant') {
        ...
        Toast.show({
          ...
          text2:
            'Restaurant confirmed your order. Please proceed with payment.',
          position: 'top',
        });
      }
  ...
  const processPayment = useCallback(
    async (paymentDetails: PaymentDetails) => {
      ...
      try {
        ...
        const result: PaymentPollingResult =
          await EnhancedPaymentService.processPaymentWithRetry(
            paymentRequest,
            (status) => {},
          );
  ...
  return {
    ...
    isPaymentProcessing:
      flowState.step === 'payment_processing' ||
      flowState.step === 'payment_polling',
