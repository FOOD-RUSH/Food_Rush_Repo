      ...
      if (response.data.status_code === 200 && response.data.data) {
        ...
        try {
          ...
          const { useRestaurantProfileStore } = await import(
            '@/src/stores/restaurantStores/restaurantProfileStore'
          );
          const { updateRestaurantProfile } =
            useRestaurantProfileStore.getState();
          ...
          updateRestaurantProfile({
            ...
            latitude: updatedProfile.latitude
              ? parseFloat(updatedProfile.latitude)
              : null,
            longitude: updatedProfile.longitude
              ? parseFloat(updatedProfile.longitude)
              : null,
          });
        } catch (error) {
          ...
          return {
            ...
            data: {
              ...
              latitude: response.data.data.latitude
                ? parseFloat(response.data.data.latitude)
                : null,
              longitude: response.data.data.longitude
                ? parseFloat(response.data.data.longitude)
                : null,
            },
          };
        }
