...
export const restaurantOrderApi = {
  ...
  getOrders: (params?: {
    ...
    console.log('🔍 [ORDER API] Fetching restaurant orders with params:', params);
    
    return apiClient.get<OrderResponse>('/orders/my-restaurant', { params })
      .then((res) => {
        console.log('✅ [ORDER API] Raw backend response:', JSON.stringify(res.data, null, 2));
  ...
  getOrdersByRestaurantId: (restaurantId: string) => {
    console.log(`🔍 [ORDER API] Fetching orders for restaurant ID: ${restaurantId}`);
    return apiClient.get<OrderResponse>(`/orders/restaurant/${restaurantId}`)
      .then((res) => {
        console.log(`✅ [ORDER API] Restaurant ${restaurantId} orders response:`, JSON.stringify(res.data, null, 2));
        return res;
      })
      .catch((error) => {
        console.error(`❌ [ORDER API] Error fetching orders for restaurant ${restaurantId}:`, error);
        throw error;
      });
  },
  ...
  getOrderById: (orderId: string) => {
    ...
    return apiClient.get<{ status_code: number; message: string; data: Order }>(
      ...
      .then((res) => {
        console.log(`✅ [ORDER API] Order ${orderId} details:`, JSON.stringify(res.data, null, 2));
        return res;
      })
      .catch((error) => {
  ...
  confirmOrder: (orderId: string) => {
    ...
    return apiClient.post(`/orders/${orderId}/confirm`)
      .then((res) => {
        console.log(`✅ [ORDER API] Order ${orderId} confirmed successfully:`, JSON.stringify(res.data, null, 2));
        return res;
      })
      .catch((error) => {
        console.error(`❌ [ORDER API] Error confirming order ${orderId}:`, error);
        throw error;
      });
  },
  ...
  rejectOrder: (orderId: string) => {
    ...
    return apiClient.post(`/orders/${orderId}/reject`)
      .then((res) => {
        console.log(`✅ [ORDER API] Order ${orderId} rejected successfully:`, JSON.stringify(res.data, null, 2));
        return res;
      })
      .catch((error) => {
        console.error(`❌ [ORDER API] Error rejecting order ${orderId}:`, error);
        throw error;
      });
  },
};
